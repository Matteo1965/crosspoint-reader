...
labels = {
    'case ReaderAction::OpenDictionary: return hu ? "Szótár" : "Dictionary";': 'case ReaderAction::OpenDictionary: return hu ? "Keresés / Szótár" : "Search / Dictionary";',
    'case ReaderAction::ToggleBookmark: return hu ? "Könyvjelző váltás" : "Toggle bookmark";': 'case ReaderAction::ToggleBookmark: return hu ? "Könyvjelző hozzáadása" : "Add bookmark";',
}
for old, new in labels.items():
    if old not in t: raise SystemExit(f'CPHUN-43 label anchor missing: {old}')
...
insert = '''    if (configured == ReaderAction::ReaderBack) {
    ...
    if (configured == ReaderAction::OpenLayoutMenu) { cphun36OpenLayout(); return true; }'''
if anchor not in r: raise SystemExit('CPHUN-43 gesture action anchor missing')
...
'''
if helper_anchor not in s: raise SystemExit('CPHUN-43 hyphen helper anchor missing')
...
spare_new = '''  const int spareSpace =
  ...
  const int hyphenMicroTotal = std::min<int>(spareSpace, static_cast<int>(hyphenMicroOpportunityCount) * 2);

  uint8_t letterSpacingPx = 0;'''
if spare_anchor not in s: raise SystemExit('CPHUN-43 spare-space anchor missing')
...
if adjust_anchor not in s: raise SystemExit('CPHUN-43 adjusted-spare anchor missing')
s = s.replace(adjust_anchor, '  const int adjustedSpareSpace = spareSpace - hyphenMicroTotal - (letterSpacingPx ? trackingExtraTotal : 0);', 1)
...
if pos < 0: raise SystemExit('CPHUN-43 LTR positioning anchor missing')
...
approved = ['ReaderAction::None','ReaderAction::PreviousPage','ReaderAction::NextPage','ReaderAction::OpenReaderMenu','ReaderAction::OpenTextSettings','ReaderAction::OpenDictionary','ReaderAction::GoHome','ReaderAction::ScreenMarginDown','ReaderAction::ScreenMarginUp','ReaderAction::ToggleBookmark','ReaderAction::OpenBookmarks','ReaderAction::Screenshot']
assert picker.count('ReaderAction::') == 12
for action in approved: assert action in picker, action
for label in ['Keresés / Szótár','Könyvjelző hozzáadása','Könyvjelzők','Képernyőkép']: assert label in ui, label
reader = Path('src/activities/reader/EpubReaderActivity.cpp').read_text(encoding='utf-8')
for needle in ['configured == ReaderAction::PreviousPage || configured == ReaderAction::NextPage','configured == ReaderAction::ReaderBack','configured == ReaderAction::OpenDictionary','configured == ReaderAction::ToggleBookmark','configured == ReaderAction::OpenBookmarks','configured == ReaderAction::Screenshot','Button::Power','Button::PageBack','Button::PageForward']: assert needle in reader, needle
parsed = Path('lib/Epub/Epub/ParsedText.cpp').read_text(encoding='utf-8')
for needle in ['hyphenMicroOpportunityCount','hyphenMicroTotal','hyphenMicroRemaining','std::min(2, hyphenMicroRemaining)']: assert needle in parsed, needle
