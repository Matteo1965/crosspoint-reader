...
# Build ID
replace("src/CPHUNBuildId.h", "CPHUN-260911-95", "CPHUN-260911-96")

# -----------------------------------------------------------------------------
# Reader menu: dynamic footer on tab band, cover label, optional Book-tab entry.
# -----------------------------------------------------------------------------
...
s = s.replace("  void drawChrome() override;\n", "  void drawChrome() override;\n  void drawFooter() override;\n")
p.write_text(s)
...
needle = '''void EpubReaderMenuActivity::drawChrome() {\n  const auto& metrics = UITheme::getInstance().getMetrics();\n  const Rect screen = UITheme::getInstance().getScreenSafeArea(renderer, true, false);\n  GUI.drawHeader(renderer, Rect{screen.x, screen.y + metrics.topPadding, screen.width, metrics.headerHeight},\n                 title.c_str());\n}\n'''
...
if needle not in s:
    raise SystemExit("reader menu drawChrome block missing")
s = s.replace(needle, insert, 1)
p.write_text(s)

# -----------------------------------------------------------------------------
# Reader return path: cover Back reopens the Book tab focused on the tab band.
# -----------------------------------------------------------------------------
p = Path("src/activities/reader/EpubReaderActivity.h")
s = p.read_text().replace("  void openReaderMenu();", "  void openReaderMenu(bool startOnBookTab = false);")
p.write_text(s)

p = Path("src/activities/reader/EpubReaderActivity.cpp")
s = p.read_text()
s = s.replace("void EpubReaderActivity::openReaderMenu() {", "void EpubReaderActivity::openReaderMenu(const bool startOnBookTab) {")
s = s.replace(
...
# -----------------------------------------------------------------------------
# Book information pages: plain paged viewer, no selectable metadata rows.
# Noto Serif 14 Regular throughout. Description preserves paragraphs and uses
# the same Hungarian extended hyphenation API as the reader/dictionary path.
# -----------------------------------------------------------------------------
