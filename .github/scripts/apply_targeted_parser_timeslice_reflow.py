...
# The scheduler patch already wrapped the background build with input/idle gates.
# Replace only the inner one-page build logic, preserving those gates and the
# long-lived fontReflowBackgroundThrottled state.
...
if count != 1:
    raise SystemExit(f"reader scheduled background build block: expected 1, found {count}")
