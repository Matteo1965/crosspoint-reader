...
# Bake RC version directly into source; final build no longer runs customization patches.
...
# CrossPoint Version pages: side buttons must follow the configured PageForward/PageBack layout.
replace_once(
    "src/activities/settings/CrossPointVersionActivity.cpp",
...
# Long lower-button screenshot. While the feature is enabled ButtonNavigator
# dispatches normal navigation on release, so a captured long hold can consume
# its release without a menu step. Existing Power+Down screenshot remains intact.
replace_once(
    "src/main.cpp",
...
# RoundedRaff home header: retain title, omit battery only on Home.
replace_once(
...
# Keep heap free until the 376px thumbnail is available.
replace_once(
...
# RoundedRaff Settings: hide version, battery +8px.
replace_once(
...
# EPUB image brightness: apply spatial white mix after every image render pass.
replace_once(
...
# Sleep-screen bitmap brightness uses the cover-filter setting in every grayscale pass.
replace_once(
...
# The final RC workflow validates/builds the baked source directly: no customization script.
workflow = Path(".github/workflows/build-hungarian-firmware-v1.6.0rc.yml")
text = workflow.read_text()
text = text.replace('''\n      - name: Apply Hungarian RC customizations\n        run: python scripts/apply_hungarian_rc_customizations.py\n''', '')
text = text.replace('grep -q "homeCoverHeight = 384"', 'grep -q "homeCoverHeight = 376"')
text = text.replace(
    'grep -q "longDownScreenshot" src/main.cpp',
    'grep -q "longDownScreenshot" src/main.cpp\n          grep -q "STR_PICTURE_FILTER" src/SettingsList.h\n          grep -q "brightnessPercentForPicture" lib/Epub/Epub/blocks/ImageBlock.cpp',
)
workflow.write_text(text)

# Header helper uses std::min.
